defmodule SearchHelper.UserInputTest do
  ...
  test "get user input" do
    ...
    assert get_input_option(1, count, options) === {:ok, "users"}
    assert get_input_option(2, count, options) === {:ok, "organizations"}
    assert get_input_option(3, count, options) === {:ok, "tickets"}
  end
end
